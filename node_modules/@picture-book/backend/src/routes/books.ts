...
import { generateAppearances } from '../services/sub-character-generator.js';
import { extractScenes } from '../services/scene-extractor.js';
import type { SceneDefinition, AppearanceDefinition } from '@picture-book/shared';
...
booksRouter.post('/generate-multi', async (req: Request, res: Response) => {
  ...
  // 3. Validate: all required roles assigned (only 'registered' roles need assignment)
  const requiredRoles = template.roles
    .filter((r) => r.required && (r.characterType ?? 'registered') === 'registered')
    .map((r) => r.role);
  ...
  try {
    ...
    // Step 2.5: Generate consistency definitions (Sub_Character_Generator + Scene_Extractor)
    let autoGeneratedAppearances: Map<string, AppearanceDefinition> | undefined;
    let sceneDefinitions: SceneDefinition[] = [];
    let pageSceneMapping: Record<number, string> = {};

    const autoGeneratedRoles = template.roles
      .filter((r) => r.characterType === 'auto_generated')
      .map((r) => ({ role: r.role, label: r.label }));

    emitMulti({ type: 'consistency_generating' });

    try {
      const [appearancesResult, scenesResult] = await Promise.all([
        autoGeneratedRoles.length > 0
          ? generateAppearances({ story, autoGeneratedRoles })
          : Promise.resolve({ appearances: new Map<string, AppearanceDefinition>() }),
        extractScenes({ story }),
      ]);

      if (appearancesResult.appearances.size > 0) {
        autoGeneratedAppearances = appearancesResult.appearances;
      }
      sceneDefinitions = scenesResult.scenes;
      pageSceneMapping = scenesResult.pageSceneMapping;
    } catch (err) {
      console.warn(
        '[generate-multi] Sub_Character_Generator / Scene_Extractor failed, continuing without definitions:',
        err instanceof Error ? err.message : String(err)
      );
    }

    emitMulti({ type: 'consistency_complete' });
    ...
    // Save consistency definitions to Firestore
    if (sceneDefinitions.length > 0 || autoGeneratedAppearances) {
      try {
        await saveConsistencyDefinitions(userId, bookId, {
          appearances: autoGeneratedAppearances,
          scenes: sceneDefinitions,
          pageSceneMapping,
        });
      } catch (err) {
        console.warn('[generate-multi] Failed to save consistency definitions:', err instanceof Error ? err.message : String(err));
      }
    }
    ...
    for (const page of story.pages) {
      ...
      // Look up scene definition for this page
      const sceneId = pageSceneMapping[page.pageNumber];
      const sceneDefinition = sceneId
        ? sceneDefinitions.find((s) => s.sceneId === sceneId)
        : undefined;
      ...
      const illustration = await generateForPageMultiCharacter(
        ...
        { userId, bookId, autoGeneratedAppearances, sceneDefinition }
      );
...
booksRouter.post('/generate', async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Step 2.5: Extract scene definitions for consistency
    let sceneDefinitions: SceneDefinition[] = [];
    let pageSceneMapping: Record<number, string> = {};

    emit({ type: 'consistency_generating' });

    try {
      const scenesResult = await extractScenes({ story });
      sceneDefinitions = scenesResult.scenes;
      pageSceneMapping = scenesResult.pageSceneMapping;
    } catch (err) {
      console.warn(
        '[generate] Scene_Extractor failed, continuing without scene definitions:',
        err instanceof Error ? err.message : String(err)
      );
    }

    emit({ type: 'consistency_complete' });
    ...
    if (sceneDefinitions.length > 0) {
      try {
        ...
        console.warn('[generate] Failed to save consistency definitions:', err instanceof Error ? err.message : String(err));
      }
    }
    ...
    for (const page of story.pages) {
      ...
      const illustration = await generateForPage(
        ...
        theme as Theme,
        {
          ...
          appearanceDescription,
          characterSheetBuffer,
          sceneDefinition: (() => {
            const sceneId = pageSceneMapping[page.pageNumber];
            return sceneId ? sceneDefinitions.find((s) => s.sceneId === sceneId) : undefined;
          })(),
        }
      );
