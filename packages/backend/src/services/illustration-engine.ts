...
import type { Theme, CharacterProfile, SceneDefinition, AppearanceDefinition } from '@picture-book/shared';
...
function buildPhotoReferencePrompt(
  ...
  const sceneBlock = sceneDefinition
    ? `\nSCENE DEFINITION (MUST match across all pages showing this location):\nLocation: ${sceneDefinition.locationName}\nKey elements: ${sceneDefinition.keyElements}\nColor palette: ${sceneDefinition.colorPalette}\nLighting: ${sceneDefinition.lighting}\nAtmosphere: ${sceneDefinition.atmosphere}${sceneDefinition.keyProps ? `\nKey props (MUST look identical on every page): ${sceneDefinition.keyProps}` : ''}\n`
    : '';
...
- No text or words in the image${sceneBlock}`;
}
...
export async function generateForPage(
  ...
  if (options.photoStoragePath) {
    ...
    const photoPrompt = buildPhotoReferencePrompt(page, profile, theme, options.appearanceDescription, options.sceneDefinition);
...
function buildMultiCharacterPrompt(
  ...
  autoGeneratedAppearances?: Map<string, AppearanceDefinition>,
  sceneDefinition?: SceneDefinition
): string {
  const themeLabel = THEME_LABELS[theme];
  ...
  // Build auto-generated character appearances section
  const autoGenLines: string[] = [];
  if (autoGeneratedAppearances) {
    for (const [role, appearance] of autoGeneratedAppearances.entries()) {
      if (pageRoles.includes(role)) {
        autoGenLines.push(`[${role}] ${appearance.name}: ${appearance.hairStyle}, ${appearance.hairColor}, ${appearance.clothing}, ${appearance.bodyType}, ${appearance.ageGroup}, ${appearance.distinguishingFeatures}`);
      }
    }
  }
  const autoGenSection = autoGenLines.length > 0
...
- No text or words in the image${autoGenSection}${sceneBlock}`;
}
...
export async function generateForPageMultiCharacter(
  ...
  pageRoles: string[],
  theme: Theme,
  options: { userId: string; bookId: string; autoGeneratedAppearances?: Map<string, AppearanceDefinition>; sceneDefinition?: SceneDefinition; openaiClient?: OpenAI; storageBucket?: StorageBucket }
): Promise<IllustrationResult> {
  const client = options.openaiClient ?? createOpenAIClient();
  const illustrationStoragePath = `users/${options.userId}/books/${options.bookId}/illustrations/page-${page.pageNumber}.png`;
  ...
  const prompt = buildMultiCharacterPrompt(page, characters, pageRoles, theme, imagePairIndex, options.autoGeneratedAppearances, options.sceneDefinition);
